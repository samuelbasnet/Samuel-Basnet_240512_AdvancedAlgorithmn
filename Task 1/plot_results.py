...
# Log-log plot to visualise growth rate vs Big-O predictions
fig2, ax = plt.subplots(figsize=(6, 4.5))
ax.loglog(ns, [float(r["bst_insert"]) for r in rows], "o-", label="BST insert")
ax.loglog(ns, [float(r["avl_insert"]) for r in rows], "s-", label="AVL insert")
ax.loglog(ns, [float(r["hash_insert"]) for r in rows], "^-", label="Hash insert")
ax.set_title("Insertion time (log-log scale)")
ax.set_xlabel("n (log scale)")
ax.set_ylabel("Time seconds (log scale)")
ax.legend()
ax.grid(alpha=0.3, which="both")
plt.tight_layout()
plt.savefig("task1_loglog.png", dpi=150)
print("Saved task1_loglog.png")
